...
- `src/repositories/`: interfacce (`AuthRepository`, `WorkspaceRepository`, `NoteRepository`,
  `TaskRepository`, `DocumentRepository`) implementate nel layer `data` di ogni app, per
  Dependency Inversion (Engineering Constitution, Articolo 4).
- `src/enums.dart`: enumerazioni condivise tra le entità.

## Stato

Auth, Workspace, Note, Task e Document hanno un'implementazione concreta lato app
(`apps/mobile`), verificata anche a livello di schema/RLS (`infrastructure/supabase`). Chat,
